...
export class CheckoutPaymentComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    this.stripe = Stripe('pk_test_51NUJnjD8GgYL0NcBJC92YmZJUzBJssbncQiBcUvfCbioieQDWRnW44zMwMAmv4VhgGec8qDlqv0keAqxFFVvr1bo005jxPwpJc')
  ...
  onChange({ error }: { error: { message: string } }) {
    if (error) {
    ...
    this.checkoutService.createOrder(orderToCreate).subscribe((order: any) => {
      ...
      this.stripe.confirmCardPayment(basket.clientSecret, {
        ...
        if (result.paymentIntent) {
          ...
          this.toastr.error('Payment has failed.');
        }
      });
    }, error => {
